...
export function MinimalPortfolio() {
  const [mode, setMode] = useState<"home" | "grid" | "details">("home")
  ...
  const onOpenDetails = (index: number) => {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center ${mode === "grid" || mode === "details" ? "bg-slate-100" : "bg-gradient-to-br from-slate-50 to-slate-100"} p-4`}>
      <div className="w-full max-w-6xl">
          ...
          <Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm">
            <CardContent className="p-8 text-center space-y-6">
              ...
              <div className="flex gap-4 justify-center pt-4">
                <Button onClick={onOpenGrid} className="bg-blue-600 hover:bg-blue-700 px-8">
                  Projects
                </Button>
        ...
        {mode === "grid" && (
          <ProjectsGrid onBack={() => setMode("home")} onOpenDetails={onOpenDetails} />
        )}

        {mode === "details" && selectedIndex !== null && (
          <ProjectDetails
            index={selectedIndex}
